		...
		if(whiteMoves) {
			if(piece == 'N') {
				for(int i = 0; i < pieceLocations.size(); i++) {
					...
					for(int i = 0; i < pieceLocations.size(); i++) {
						Location loc = pieceLocations.get(i);
						if(((loc.row+1==row&&loc.column==realColumn&&pieces[realColumn][row]==' ')||(loc.row+2==row&&loc.column==realColumn&&pieces[realColumn][row]==' '&&loc.row==1)||(loc.row+1==row&&loc.column+1==realColumn&&pieces[realColumn][row]!=' ')||(loc.row+1==row&&loc.column-1==realColumn&&pieces[realColumn][row]!=' '))&&!validPieces.contains(String.valueOf(pieces[realColumn][row]))) {
							/*if(loc.row==1&&row==2+loc.row) {
				...
				for(int i = 0; i < pieceLocations.size(); i++) {
					Location loc = pieceLocations.get(i);
					for(int j = 1; j <= 7; j++) {
						if(((loc.column+j==realColumn&&loc.row+j==row)||(loc.column-j==realColumn&&loc.row+j==row)||(loc.column-j==realColumn&&loc.row-j==row)||(loc.column+j==realColumn&&loc.row-j==row))&&!validPieces.contains(String.valueOf(pieces[realColumn][row]))){
				...
				for(int i = 0; i < pieceLocations.size(); i++) {
					Location loc = pieceLocations.get(i);
					for(int j = 1; j <= 7; j++) {
						if(((loc.column+j==realColumn&&loc.row==row)||(loc.column-j==realColumn&&loc.row==row)||(loc.column==realColumn&&loc.row-j==row)||(loc.column==realColumn&&loc.row+j==row))&&!validPieces.contains(String.valueOf(pieces[realColumn][row]))){
							boolean canMove = false;							 
							for(int k = 1; k < j; k++) {
								if(k==j-1||j==1) {
									canMove = true;
			...
			if(piece == 'n' ) {
				...
				for(int i = 0; i < pieceLocations.size(); i++) {
					Location loc = pieceLocations.get(i);
					if(((loc.row-1==row&&loc.column==realColumn&&pieces[realColumn][row]==' ')||(loc.row-2==row&&loc.column==realColumn&&pieces[realColumn][row]==' ')&&loc.row==6||(loc.row-1==row&&loc.column+1==realColumn&&pieces[realColumn][row]!=' ')||(loc.row-1==row&&loc.column-1==realColumn&&pieces[realColumn][row]!=' '))&&!validBlackPieces.contains(String.valueOf(pieces[realColumn][row]))) {
						pieces[loc.column][loc.row] = ' ';
						pieces[realColumn][row] = piece;
						if(King.isInCheck(pieces, whiteMoves)) {
							pieces[realColumn][row] = pieceAt;
				...
				for(int i = 0; i < pieceLocations.size(); i++) {
					Location loc = pieceLocations.get(i);
					for(int j = 1; j <= 7; j++) {
						if(((loc.column+j==realColumn&&loc.row+j==row)||(loc.column-j==realColumn&&loc.row+j==row)||(loc.column-j==realColumn&&loc.row-j==row)||(loc.column+j==realColumn&&loc.row-j==row))&&!validBlackPieces.contains(String.valueOf(pieces[realColumn][row]))){
							boolean canMove = false;							 
							for(int k = 1; k < j; k++) {
								if(k==j-1||j==1) {
									canMove=true;
				...
				for(int i = 0; i < pieceLocations.size(); i++) {
					Location loc = pieceLocations.get(i);
					for(int j = 1; j <= 7; j++) {
						if(((loc.column+j==realColumn&&loc.row==row)||(loc.column-j==realColumn&&loc.row==row)||(loc.column==realColumn&&loc.row-j==row)||(loc.column==realColumn&&loc.row+j==row))&&!validBlackPieces.contains(String.valueOf(pieces[realColumn][row]))){
							boolean canMove = false;							 
							for(int k = 1; k < j; k++) {
								if(k==j-1||j==1) {
									canMove = true;
				...
				for(int i = 0; i < pieceLocations.size(); i++) {
					Location loc = pieceLocations.get(i);
					for(int j = 1; j <= 7; j++) {
						if(((loc.column+j==realColumn&&loc.row==row)||(loc.column-j==realColumn&&loc.row==row)||(loc.column==realColumn&&loc.row-j==row)||(loc.column==realColumn&&loc.row+j==row)||(loc.column+j==realColumn&&loc.row+j==row)||(loc.column-j==realColumn&&loc.row+j==row)||(loc.column-j==realColumn&&loc.row-j==row)||(loc.column+j==realColumn&&loc.row-j==row))&&!validBlackPieces.contains(String.valueOf(pieces[realColumn][row]))){
							boolean canMove = false;							 
							for(int k = 1; k < j; k++) {
								if(k==j-1||j==1) {
									canMove = true;
